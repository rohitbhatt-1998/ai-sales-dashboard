...
// Find the call log
$log = DB::fetchOne('SELECT * FROM call_logs WHERE call_sid = ?', [$callSid]);
if ($log) {
    DB::execute(
        'UPDATE call_logs SET status = ?, duration = ?, ended_at = NOW() WHERE call_sid = ?',
        [$ourStatus, $duration, $callSid]
    );
